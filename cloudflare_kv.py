...
class Namespace:
    def __init__(self, account_id: str, namespace_id: str, api_key: str):
        ...
        self.request_base_url = f"https://api.cloudflare.com/client/v4/accounts/{self.account_id}/storage/kv/namespaces/{self.namespace_id}"
    ...
    def delete_multiple(self, keys: list):
        warnings.warn(
            "delete_multiple() is deprecated and should be removed in version 2.0. Use delete_many() instead.",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    def _write_many(self, key_value_pairs: dict):
        ...
        for key in key_value_pairs:
            value = (
                json.dumps(key_value_pairs[key])
                if type(key_value_pairs[key]) in (list, dict)
                else str(key_value_pairs[key])
            )
            data.append({"key": str(key), "value": value})
        response = requests.put(url, headers=self.request_headers, json=data)
        if response.json().get("success") is not True:
            raise Exception(response.json()["errors"])
        return None
